...
## Composer v2 scoring focus

The composer uses trigger-specific routing, context-grounded facts, merchant-state hooks, category-aware framing, source/date extraction, customer preferences, single-CTA output, language adaptation, and deterministic anti-repetition. It never invents facts when the corresponding context is absent.


## Final submission preparation

Run `./prepare_final_submission.sh` from the project folder after generating the challenge's expanded dataset. It regenerates `submission.jsonl` from the canonical `test_pairs.json`, checks that exactly 30 rows exist, and performs a syntax check before deployment.
